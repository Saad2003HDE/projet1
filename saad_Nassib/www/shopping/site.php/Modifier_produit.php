...
if (isset($_GET['id']) && !empty($_GET['id'])) {
    ...
    $recupProduit = $pdo->prepare('SELECT * FROM product WHERE id = ?');
    ...
    if ($recupProduit->rowCount() > 0) {
        ...
        $name = $produitInfos['name'];
        $description = str_replace('<br />', "\n", $produitInfos['description']);
        $img_url = $produitInfos['img_url'];
        $quantity = $produitInfos['quantity'];
        $price = $produitInfos['price'];
        ...
        if (isset($_POST['valider'])) {
            ...
            $updateProduit = $pdo->prepare('UPDATE product SET name = ?, description = ?, img_url = ?, quantity = ?, price = ? WHERE id = ?');
            $updateProduit->execute(array($name_saisi, $description_saisi, $img_url_saisi, $quantity_saisi, $price_saisi, $getid));

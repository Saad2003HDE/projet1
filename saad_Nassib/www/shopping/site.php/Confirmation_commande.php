...
if (isset($_SESSION['user_id'])) {
    ...
    $addressStmt->execute([$user_id, $user_id]);
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $stmt->execute([$orderRef, $orderDate, $totalCartPrice, $user_id]);
        ...
        VALUES (?, ?, ?, ?, ?, ?)";
        $deliveryAddressStmt = $pdo->prepare($deliveryAddressSql);
        $deliveryAddressStmt->execute([$delivery_street_name, $delivery_street_nb, $delivery_city, $delivery_province, $delivery_zip_code, $delivery_country]);
        ...
        VALUES (?, ?, ?, ?, ?, ?)";
        $paymentAddressStmt = $pdo->prepare($paymentAddressSql);
        $paymentAddressStmt->execute([$payment_street_name, $payment_street_nb, $payment_city, $payment_province, $payment_zip_code, $payment_country]);
        $paymentAddressId = $pdo->lastInsertId();
        ...
        $updateOrderStmt->execute([$deliveryAddressId, $paymentAddressId, $user_id]);


        // Assuming you have a cart table with columns: user_id, product_id, quantity
        $deleteCartItemsSql = "DELETE FROM order_has_product WHERE order_id = ?";
        $stmt = $pdo->prepare($deleteCartItemsSql);
        $stmt->execute([$user_id]);
